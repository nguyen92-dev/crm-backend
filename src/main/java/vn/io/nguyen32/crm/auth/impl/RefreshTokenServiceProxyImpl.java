...
public class RefreshTokenServiceProxyImpl implements IRefreshTokenServiceProxy {
    ...
    IRefreshTokenService<
        LogInResDto.LogInResDtoBuilder
    > refreshTokenResponseService;

    @Override
    public void addRefreshToken(Object response, String refreshToken) {
        switch (response) {
            case HttpServletResponse servletResponse -> refreshTokenCookieService.addRefreshToken(
                servletResponse,
                refreshToken
            );
            case LogInResDto.LogInResDtoBuilder dtoBuilder -> refreshTokenResponseService.addRefreshToken(
                dtoBuilder,
                refreshToken
            );
    ...
    public void clearRefreshToken(Object response) {
        switch (response) {
            case HttpServletResponse servletResponse -> refreshTokenCookieService.clearRefreshToken(
                servletResponse
            );
            case LogInResDto.LogInResDtoBuilder dtoBuilder -> refreshTokenResponseService.clearRefreshToken(
                dtoBuilder
            );
            default -> throw new IllegalArgumentException(
                "Invalid response type"
