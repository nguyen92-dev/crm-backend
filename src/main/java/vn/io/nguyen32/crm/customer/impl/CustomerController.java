...
public class CustomerController implements CustomerContract {
    ...
    public ResponseEntity<
        BaseResponse<PagedModel<CustomerResDTO>>
    > getPagedList(Pageable pageable, FilterReqDto filter) {
        return doGetPagedList(pageable, filter);
    }

    @Override
    public ResponseEntity<BaseResponse<CustomerResDTO>> createCustomer(
        CustomerReqDTO reqDto
    ) {
        CustomerResDTO result = customerService.createCustomer(reqDto);
        URI uri = URI.create("api/v1/user/%s".formatted(result.id()));
        return ResponseEntity.created(uri).body(
            BaseResponse.buildSuccess(result)
        );
    }
    ...
    @Override
    public ResponseEntity<BaseResponse<CustomerResDTO>> updateCustomer(
        Long id,
        CustomerReqDTO reqDto
    ) {
